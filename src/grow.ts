...
export async function main(ns: NS): Promise<void>  {
  ...
  // ns.print(`Growing ${target} with ${threads} thread(s) with an offset of ${msOffset}ms`);

  const start = new Date().getTime();
  const result = await ns.grow(target, opts);
  const duration = (new Date().getTime() - start) - msOffset;

  // ns.print(`Grew ${target} ${ns.formatNumber(result)}x in ${ns.tFormat(duration)}`);
}