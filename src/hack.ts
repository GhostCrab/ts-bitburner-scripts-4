...
export async function main(ns: NS): Promise<void>  {
  ...
  // ns.print(`Hacking ${target} with ${threads} thread(s) with an offset of ${msOffset}ms`);

  const start = new Date().getTime();
  const result = await ns.hack(target, opts);
  const duration = (new Date().getTime() - start) - msOffset;

  // ns.print(`Hacked ${ns.formatNumber(result, 3, 1000, true)}$ from ${target} in ${ns.tFormat(duration)}`);
}